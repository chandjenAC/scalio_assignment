...
const AnchorSettingsContainer = (props) => {
  const { resource, lastSelectedNode } = props;

  const getAnchorSettings = (query) => {
    let body = {
      entityId: lastSelectedNode?.id,
    };
    return new Promise((resolve, reject) => {
      post(env.NEON_SERVICE_SEARCH_ANCHOR_SETTINGS, body).then((result) => {
  ...
  const addSettings = async (newData) => {
    await post(env.NEON_SERVICE_SAVE_ANCHOR_SETTINGS, newData);
  };
  ...
  const deleteSettings = async (oldData) => {
    await del(env.NEON_SERVICE_SAVE_ANCHOR_SETTINGS, oldData);
  };
...
export default AnchorSettingsContainer;
