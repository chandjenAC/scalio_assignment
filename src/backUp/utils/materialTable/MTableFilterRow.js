...
class MTableFilterRow extends React.Component {
  ...
  renderBooleanFilter = (columnDef) => (
    <Checkbox
      ...
      checked={columnDef.tableData.filterValue === true}
      onChange={() => {
        let val;
        if (columnDef.tableData.filterValue === undefined) {
          val = true;
        } else if (columnDef.tableData.filterValue === true) {
          val = false;
        }
  ...
  render() {
    const columns = this.props.columns
      .filter(
        (columnDef) => !columnDef.hidden && !this.checkGroupOrder(columnDef)
      )
